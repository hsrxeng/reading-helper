          ...
          contents: [
            {
              ...
              parts: [
                ...
                {
                  text: `이 영어 교재/지문/시험지/원서 사진에서 영어 본문 내용과 문제 정보를 정밀하게 추출하여 반드시 아래 JSON 형식으로만 반환해 주세요.
Markdown 코드 블록이나 기타 주석 없이 순수 JSON만 반환하세요:
{
  "passage": "영어 지문 본문 전체 (오탈자 없이 원문 철자와 문장부호 유지, 줄바꿈으로 끊어진 단어는 연결, 교재/본문/시험지 전체 글)",
  "questionPrompt": "문제 발문 (예: 다음 글의 밑줄 친 부분 중 어법상 틀린 것은? 또는 제목/빈칸 발문 등. 문제 발문이 없는 일반 교재/지문이면 빈 문자열)",
  "choices": ["1번 선지", "2번 선지", "3번 선지", "4번 선지", "5번 선지 (선지가 없는 일반 지문이나 서술형이면 빈 배열 [])"],
  "questionType": "추정 문항 유형 (예: 어법상 틀린 것, 낱말의 쓰임, 빈칸추론, 제목 등. 일반 지문이면 빈 문자열)",
